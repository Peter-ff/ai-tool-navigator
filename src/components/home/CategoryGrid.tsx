...
export function CategoryGrid() {
  return (
    <section>
      <h2 className="mb-5 text-[21px] font-semibold tracking-tight text-apple-text">热门分类</h2>
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {CATEGORIES.map((cat) => (
          <Link
            key={cat.slug}
            href={`/category/${cat.slug}`}
            className="group flex flex-col items-center gap-2 rounded-2xl bg-white px-3 py-5 transition-all duration-200 ease-out hover:shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 border border-transparent hover:border-black/5"
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-apple-bg text-apple-blue transition-colors group-hover:bg-apple-blue group-hover:text-white">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d={ICON_SVG[cat.slug]} />
              </svg>
            </div>
            <span className="text-[12px] font-medium text-apple-tertiary group-hover:text-apple-text transition-colors">
              {cat.name}
            </span>
          </Link>
        ))}
